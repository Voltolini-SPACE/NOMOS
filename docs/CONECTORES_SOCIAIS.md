...
| **Slack** | ✅ envio pronto (`examples/mcp/slack/`) — texto para um canal, via **Incoming Webhook oficial** | criar um Incoming Webhook no workspace e apontar `NOMOS_SLACK_WEBHOOK` (a URL é secreta) | só envio (receber exigiria app/socket permanente); **recusa destino fora de `hooks.slack.com`**; URL redigida em erros, mascarada em `quem_sou` |
