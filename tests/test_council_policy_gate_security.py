"""MC6 — segurança/pureza do gate dry-run.

Prova por AST que `policy_gate` não importa rede/subprocess/threading/asyncio/
SDK cloud/motor, não toca FS, não usa env/time/random, e não chama
policy/vault/audit/approval reais.
"""
import ast

from nomos.council import policy_gate


def _imports():
    src = open(policy_gate.__file__, encoding="utf-8").read()
    nomes = set()
...
def test_policy_gate_module_does_not_import_network():
...
def test_policy_gate_module_does_not_import_subprocess_threading_asyncio():
    proibidos = {"subprocess", "threading", "multiprocessing", "asyncio",
                 "concurrent.futures", "signal"}
    assert not (_imports() & proibidos), _imports() & proibidos


def test_policy_gate_module_does_not_import_cloud_clients():
    usados = _imports()
...
def test_policy_gate_module_does_not_import_runtime_engines():
    usados = _imports()
...
def test_policy_gate_module_does_not_touch_filesystem_or_env():
    usados = _imports()
...
def test_policy_gate_module_does_not_use_time_or_random():
    usados = _imports()
...
def test_policy_gate_does_not_call_policy_vault_audit_approval():
    usados = _imports()
...
def test_policy_gate_only_stdlib_and_models():
    usados = _imports()
...
def test_policy_gate_no_approval_or_enable_api():
    proibidos = {"approve", "request_approval", "enable", "activate",
                 "mutate_policy", "set_policy"}
    assert not (set(dir(policy_gate)) & proibidos)
